...
# 环境变量定义
readonly RESULT_PATH="/home/ubuntu20/Workspace/Datasets/12Events/20s/20_per_1_time"  # 结果保存路径
...
run_ELF() {
    ...
    for file in "${elf_files[@]}"; do
        ...
        log_msg "已复制文件 ${file} 从 ${PURE_ELF_PATH} 到 ${ELF_PATH_LOCAL}"
        ...
        log_msg "已删除文件 ${file} 从 ${ELF_PATH_LOCAL}"
    done
}
...
main() {
    ...
    # 实验1: 20秒持续时间,不分段采样
    log_msg "开始第一组实验. 持续时间:20s 无间隔"
    run_experiment "20s" "" "20"
    log_msg "完成第一组实验"
    
    # 实验2: 4秒持续时间,重复5次
    log_msg "开始第二组实验. 持续时间:4s 无间隔 重复:5次"
    for i in {1..5}; do
        log_msg "第二组实验第${i}次"
        run_experiment "4s/4s_${i}" "" "4"
    done
    log_msg "完成第二组实验"
    
    # 实验3: 20秒持续时间,200ms间隔采样
    log_msg "开始第三组实验. 持续时间:20s 间隔:200ms"
    run_experiment "20s_200ms" "-I 200" "20"
    log_msg "完成第三组实验"
    
    # 实验4: 20秒持续时间,30ms间隔采样
    log_msg "开始第四组实验. 持续时间:20s 间隔:20ms"
    run_experiment "20s_20ms" "-I 20" "20"
    log_msg "完成第四组实验"
}
